...
class Player(Character):
    def __init__(self, screen, pos, *groups):
        super().__init__(pos, PLAYER_DAMAGE, BATARANG_RANGE, groups)
        self.screen = screen
        self.pos = pos
        self.jump_release = 0
        self.crouching = False
        self.goal_reached = False
        self.seen_enemy = None
        self.seen_obstacle = None
        self.score = 0
        self.tmp_acc = [0, 0]
    ...
    def move(self):
        self.acc.x = self.tmp_acc[0]
        # self.acc.y = self.tmp_acc[1]
        self.tmp_acc[0] = 1
    ...
    def act(self, action):
        ...
        if action == 'left' and not self.crouching:
            self.direction = 'L'
            if not self.attack_count > 0 and not self.active_name == 'batarang_throw':
                self.tmp_acc[0] = -PLAYER_ACC

        if action == 'right' and not self.crouching:
            self.direction = 'R'
            if not self.attack_count > 0 and not self.active_name == 'batarang_throw':
                self.tmp_acc[0] = PLAYER_ACC

        # jumping
        if action == 'jump' and not self.crouching:
    ...
    def enemy_in_range(self, enemy_pos):
        enemy_x, enemy_y = enemy_pos.x, enemy_pos.y
        distance_to_enemy = abs(enemy_x - self.pos.x)
        return distance_to_enemy <= self.range and abs(enemy_y-self.pos.y) <= self.image.get_rect().height / 1.5

    def saw_enemy(self, enemy_pos):
        enemy_x, enemy_y = enemy_pos.x, enemy_pos.y
        enemy_in_range = self.enemy_in_range(enemy_pos)
        is_facing_enemy = any([self.direction == 'L' and enemy_x <= self.pos.x,
                                self.direction == 'R' and enemy_x >= self.pos.x])

        return enemy_in_range, is_facing_enemy


    def obstacle_in_range(self, obstacle_pos):
        obstacle_x, obstacle_y = obstacle_pos.x, obstacle_pos.y
        distance_to_obstacle = abs(obstacle_x - self.pos.x)
        return distance_to_obstacle <= self.range // 4 and abs(obstacle_y-self.pos.y) <= self.image.get_rect().height / 1.5

    def saw_obstacle(self, obstacle_pos):
        obstacle_x, obstacle_y = obstacle_pos.x, obstacle_pos.y
        obstacle_in_range = self.obstacle_in_range(obstacle_pos)
        is_facing_obstacle = any([self.direction == 'L' and obstacle_x <= self.pos.x,
                                self.direction == 'R' and obstacle_x >= self.pos.x])

        return obstacle_in_range, is_facing_obstacle
    ...
    def update(self, **kwargs):
        ...
        if self.seen_obstacle is None:
            for obstacle in obstacles:
                obstacle_in_range, is_facing_obstacle = self.saw_obstacle(obstacle.pos)
                if obstacle_in_range and is_facing_obstacle:
                    self.seen_obstacle = obstacle
                    break
        else:
            obstacle_x, obstacle_y = self.seen_obstacle.pos.x, self.seen_obstacle.pos.y
            is_facing_obstacle = any([self.direction == 'L' and obstacle_x <= self.pos.x,
                                      self.direction == 'R' and obstacle_x >= self.pos.x])
            if not is_facing_obstacle:
                self.seen_obstacle = None

        if self.seen_enemy is None:
            for enemy in enemies:
                enemy_in_range, is_facing_enemy = self.saw_enemy(enemy.pos)
                if enemy_in_range and is_facing_enemy:
                    self.seen_enemy = enemy
                    break
        else:
            if self.seen_enemy.is_dead() or not self.enemy_in_range(self.seen_enemy.pos):
                self.seen_enemy = None
    ...
    def get_data(self):
        data = []
        data.append(self.rect.centerx)
        data.append(self.rect.centery)
        data.append(self.rect.h)
        data.append(int(self.direction == 'R'))
        data.append(int(self.jump_release > 0 and self.ground_count > 0))  # Can jump
        data.append(int(self.attack_count == 0))  # Can attack

        data.append(int(self.seen_enemy.saw_player(self.pos)) if self.seen_enemy else -1)
        data.append(self.seen_enemy.rect.centerx if self.seen_enemy else 0)
        data.append(self.seen_enemy.rect.centery if self.seen_enemy else 0)
        data.append(self.seen_enemy.rect.h if self.seen_enemy else 0)

        data.append(self.seen_obstacle.rect.centerx if self.seen_obstacle else 0)
        data.append(self.seen_obstacle.rect.centery if self.seen_obstacle else 0)
        data.append(self.seen_obstacle.rect.h if self.seen_obstacle else 0)
...
class MeleeEnemy(Character):
    def __init__(self, screen, pos, right_boundary_x, left_boundary_x, *groups):
        super().__init__(pos, MELEE_ENEMY_DAMAGE, KNIFE_RANGE, groups)
        self.screen = screen
        self.pos = pos
